    ...
    <div v-if="juStore.ju.refprofil && juStore.ju.refprofil.length > 0" class="ref">
      ...
      <div class="options-container">
        <div v-for="option in availableOptions" :key="option.id" class="option-item">
          ...
          <div
            v-if="option.id === 'telefonnummer' && selectedOptions.telefonnummer"
            class="telefon-type-selection"
          >
            <h5>Telefontyp auswählen:</h5>
            <div class="telefon-types">
              <label v-for="type in telefonTypes" :key="type.id" class="telefon-type-option">
                <input
                  type="radio"
                  :value="type.id"
      ...
      <div class="navigation-buttons">
        <button @click="$emit('go-back')" class="back-button">← {{ backButtonText }}</button>
        <button @click="$emit('show-summary')" class="next-button">Zusammenfassung →</button>
      </div>
      <hr class="shadow-line" />
    </div>
    <div v-else>
...
onMounted(async () => {
  await Promise.all([fetchOptions(), fetchTelefonTypes()])
