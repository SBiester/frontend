  ...
  <div>
    <div v-if="hasAnyData" class="ref">
      ...
      <div class="warning-container">
        <p class="warning">
          Vergewissere dich noch einmal, dass alle von dir eingegebenen Daten korrekt sind.
        </p>
      </div>
      ...
      <div class="summary-section" v-if="ju.refprofil && ju.refprofil.length > 0">
        ...
        <div class="summary-content">
          <div v-for="profil in ju.refprofil" :key="profil.id" class="profile-container">
            ...
            <div
              v-if="profileHardware[profil.name] && profileHardware[profil.name].length > 0"
              class="profile-items"
            >
              <h5>Hardware aus diesem Profil:</h5>
              <div class="profile-item-list">
                <div
                  v-for="item in profileHardware[profil.name]"
                  :key="`hw-${item.id}`"
                  class="summary-item hardware-item profile-sub-item"
                >
            ...
            <div
              v-if="profileSoftware[profil.name] && profileSoftware[profil.name].length > 0"
              class="profile-items"
            >
              <h5>Software aus diesem Profil:</h5>
              <div class="profile-item-list">
                <div
                  v-for="item in profileSoftware[profil.name]"
                  :key="`sw-${item.id}`"
                  class="summary-item software-item profile-sub-item"
                >
                  <span class="item-name">{{ item.name }}</span>
      ...
      <div class="summary-section" v-if="ju.additionalHardware && ju.additionalHardware.length > 0">
        ...
        <div class="summary-content">
          <div
            v-for="item in ju.additionalHardware"
            :key="item.id"
            class="summary-item hardware-item"
          >
      ...
      <div class="summary-section" v-if="ju.additionalSoftware && ju.additionalSoftware.length > 0">
        ...
        <div class="summary-content">
          <div
            v-for="item in ju.additionalSoftware"
            :key="item.id"
            class="summary-item software-item"
          >
            <span class="item-name">{{ item.name }}</span>
      ...
      <div class="summary-section" v-if="ju.sapProfiles && ju.sapProfiles.length > 0">
        ...
        <div class="summary-content">
          <div
            v-for="profile in ju.sapProfiles"
            :key="profile.id"
            class="summary-item sap-berechtigung-item"
          >
            ...
            <div v-if="ju.additionalOptions.telefontyp" class="profile-items">
              ...
              <div class="profile-item-list">
                <div class="summary-item profile-sub-item">
                  <span class="item-name">{{
                    getTelefonTypeName(ju.additionalOptions.telefontyp)
                  }}</span>
                </div>
              </div>
            </div>
      ...
      <div class="navigation-buttons">
        <button @click="$emit('go-back')" class="back-button">← Optionale Services</button>
        <button @click="submitJobUpdate" class="submit-button">Job-Update abschicken</button>
      </div>
      <hr class="shadow-line" />
    </div>
    <div v-else>
  ...
  for (const profil of ju.value.refprofil) {
    try {
      ...
      const hardwareResponse = await fetch(
        `/api/hardware/profile?profile=${encodeURIComponent(profil.name)}`,
      )
      ...
      const softwareResponse = await fetch(
        `/api/software/profile?profile=${encodeURIComponent(profil.name)}`,
      )
