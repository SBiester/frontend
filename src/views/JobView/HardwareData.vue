  ...
  <div>
    <div v-if="juStore.ju.refprofil && juStore.ju.refprofil.length > 0" class="ref">
      ...
      <div class="hardware-selection">
        <div class="search-section">
          ...
          <div v-if="searchQuery && searchResults !== null" class="search-results-info">
            ...
            <span v-else> Keine Hardware gefunden für "{{ searchQuery }}" </span>
        ...
        <div class="hardware-lists">
              ...
              >
                ...
                <div class="list-group">
                  <div v-for="element in category.items" :key="element.id" class="list-group-item">
                    ...
                    <button @click="addHardwareItem(element)" class="add-button fa">
                      &#10010;
                    </button>
                  </div>
                </div>
          ...
          <div v-else-if="selectedCategory" class="source-list">
            ...
            <draggable class="list-group" :list="sourceHardwareList" group="hardware" item-key="id">
                ...
                <template #item="{ element, index }">
                  <div class="list-group-item">
                    ...
                    <div class="quantity-controls">
                      <button
                        @click="removeHardware(index)"
                        class="quantity-button remove-quantity"
                        title="Menge verringern/Entfernen"
                      >
                        -
                      </button>
                      <span class="quantity-display">{{ element.quantity || 1 }}</span>
                      <button
                        @click="addHardwareItem(element)"
                        class="quantity-button add-quantity"
                        title="Menge erhöhen"
                      >
                        +
                      </button>
      ...
      <div class="navigation-buttons">
        <button @click="$emit('go-back')" class="back-button">← Referenzprofil</button>
        <button @click="$emit('show-software')" class="next-button">Software →</button>
      </div>
      <hr class="shadow-line" />
    </div>
    <div v-else>
...
const fetchHardwareByCategory = async (category: string) => {
  try {
    ...
    const filteredHardware = data.data.filter(
      (item) => item.category === category && !assignedIds.includes(item.id),
    )
    ...
    console.log(
      `Hardware loaded for category ${category} (excluding assigned):`,
...
const addHardwareItem = (item: any) => {
  ...
  const existingIndex = additionalHardwareList.value.findIndex(
    (hardwareItem) => hardwareItem.id === item.id,
  )

  if (existingIndex > -1) {
    // Item exists, increment quantity
    additionalHardwareList.value[existingIndex].quantity =
      (additionalHardwareList.value[existingIndex].quantity || 1) + 1
    console.log(
      `Hardware Menge erhöht: ${item.name} (${additionalHardwareList.value[existingIndex].quantity})`,
    )
  } else {
