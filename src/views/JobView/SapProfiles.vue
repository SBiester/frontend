<template>
  <div>
    <div v-if="juStore.ju.refprofil && juStore.ju.refprofil.length > 0" class="sap-profiles">
      ...
      <div
        v-if="selectedProfiles.some((p) => p.fromReferenceProfile)"
        class="reference-profiles-section"
      >
      ...
      <div class="search-section">
        ...
        <div v-if="searchQuery && searchResults !== null" class="search-results-info">
          ...
          <span v-else> Keine Profile gefunden für "{{ searchQuery }}" </span>
      ...
      <div class="navigation-buttons">
        <button @click="goBack" class="back-button">← Software</button>
        <button @click="continueToOptions" class="next-button">Optionale Services →</button>
      </div>
      <hr class="shadow-line" />
    </div>
    <div v-else>
...
const currentProfileGroups = computed(() => {
  const groups =
    searchQuery.value && searchResults.value ? searchResults.value.groups : profileGroups.value
