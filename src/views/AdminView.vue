<template>
  ...
  <div v-else class="admin-view-container">
    ...
    <div class="navigation-buttons-fixed">
      ...
      <button
        v-if="!isDirectOrderAccess && userStore.canAccessAdmin"
        @click="activeComponent = 'AdminDashboard'"
        class="dashboard-button-header"
      >
        Dashboard
      </button>
      ...
      <div
        ...
        v-if="
          activeComponent !== 'AdminDashboard' && !isDirectOrderAccess && userStore.canAccessAdmin
        "
      >
    ...
    <div :class="{ 'fade-in': showWorkArea }" class="new-div">
      ...
      <AdminDashboard
        v-if="activeComponent === 'AdminDashboard' && userStore.canAccessAdmin"
      ...
      <AdminUsers
        v-if="activeComponent === 'AdminUsers' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <AdminRoles
        v-if="activeComponent === 'AdminRoles' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <AdminHardware
        v-if="activeComponent === 'AdminHardware' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <AdminSoftware
        v-if="activeComponent === 'AdminSoftware' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <AdminProfiles
        v-if="activeComponent === 'AdminProfiles' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <AdminMasterData
        v-if="activeComponent === 'AdminMasterData' && userStore.canAccessAdmin"
        @go-back="activeComponent = 'AdminDashboard'"
      />

      <!-- AdminOrders für alle berechtigten User (Admin, PM, Fach) -->
      <AdminOrders
        v-if="activeComponent === 'AdminOrders' && (userStore.canAccessAdmin || canAccessOrders)"
...
const canAccessOrders = computed(() => {
  ...
  return (
    isDirectOrderAccess.value &&
    ['pm', 'fach'].includes(userStore.role) &&
    route.query.section === 'orders'
  )
})
